...
public class TestDatastratoTopicOperationDispatcher extends TestDatastratoOperationDispatcher {
  static DatastratoSchemaOperationDispatcher schemaOperationDispatcher;
  static DatastratoTopicOperationDispatcher topicOperationDispatcher;
  ...
  @Test
  public void testCreateAndListTopics() throws IOException {
    ...
    // test listEntities
    Optional<TopicEntity> entity =
        topicOperationDispatcher.listEntities(topicNs).stream()
            .filter(e -> e.name().equals(topicIdent1.name()))
            .findFirst();
    Assertions.assertTrue(entity.isPresent());
    TopicEntity topicEntity =
        entityStore.get(topicIdent1, Entity.EntityType.TOPIC, TopicEntity.class);
    Assertions.assertEquals(topicEntity, entity.get());

    // Test listEntities with a non-existent namespace
    Namespace nonExistentNs = Namespace.of(metalake, catalog, "nonExistent");
    Exception exception =
        Assertions.assertThrows(
            NoSuchSchemaException.class,
            () -> topicOperationDispatcher.listEntities(nonExistentNs));
