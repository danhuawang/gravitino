...
public class TestDatastratoSchemaOperationDispatcher extends TestDatastratoOperationDispatcher {
  static DatastratoSchemaOperationDispatcher dispatcher;

  @BeforeAll
  public static void initialize() throws IllegalAccessException {
    dispatcher = new DatastratoSchemaOperationDispatcher(catalogManager, entityStore, idGenerator);
  ...
  public void testCreateAndListSchemas() throws IOException {
    ...
    // test listEntities
    Optional<SchemaEntity> entity =
        dispatcher.listEntities(ns).stream().filter(s -> s.name().equals("schema1")).findFirst();
    Assertions.assertTrue(entity.isPresent());
    Assertions.assertEquals(schemaEntity, entity.get());

    // test listEntities with non-existent namespace
    Namespace nonExistentNs = Namespace.of(metalake, catalog, "nonExistent");
    Exception exception =
        Assertions.assertThrows(
            NoSuchCatalogException.class, () -> dispatcher.listEntities(nonExistentNs));
    Assertions.assertEquals(
        "Catalog does not exist: metalake.catalog.nonExistent", exception.getMessage());
