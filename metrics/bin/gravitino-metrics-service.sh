...
#
#set -ex
USAGE="-e Usage: bin/gravitino-metrics-service.sh.template.sh [--config <conf-dir>]\n\t
...
function found_gravitino_metrics_service_pid() {
  process_name='GravitinoMetricsService';
  ...
  if ! kill -0 ${RUNNING_PIDS} > /dev/null 2>&1; then
    echo "GravitinoMetricsService running but process is dead"
  fi
...
function check_process_status() {
  local pid=$(found_gravitino_metrics_service_pid)

  if [[ -z "${pid}" ]]; then
    echo "GravitinoMetricsService is not running"
  else
    printArt
    echo "GravitinoMetricsService is running[PID:$pid]"
  fi
}

function wait_for_gravitino_metrics_service_to_die() {
  timeout=10
  timeoutTime=$(date "+%s")
...
function start() {
  ...
  if [[ ! -z "${pid}" ]]; then
    if kill -0 ${pid} >/dev/null 2>&1; then
      echo "GravitinoMetricsService is already running"
      return 0;
    fi
  fi
  ...
  pid=$!
  if [[ -z "${pid}" ]]; then
    echo "GravitinoMetricsService start error!"
    return 1;
  fi
...
function stop() {
  ...
  pid=$(found_gravitino_metrics_service_pid)

  if [[ -z "${pid}" ]]; then
    echo "GravitinoMetricsService is not running"
  else
    wait_for_gravitino_metrics_service_to_die
    echo "GravitinoMetricsService stop"
  fi
}

HOSTNAME=$(hostname)
GRAVITINO_OUTFILE="${GRAVITINO_LOG_DIR}/gravitino-metrics-service.out"
GRAVITINO_SERVER_NAME=com.datastrato.gravitino.metrics.GravitinoMetricsService
GRAVITINO_SIMPLE_SERVER_NAME=gravitino-metrics-service
