...
public interface ScimUserGroupRelMapper {
  ...
  /**
   * Selects active members for the given group ids in one query.
   *
   * @param groupIds group ids; empty or {@code null} yields no rows
   * @return member rows including each membership group id
   */
  @SelectProvider(
      type = ScimUserGroupRelSQLProviderFactory.class,
      method = "selectMembersByGroupIds")
  List<ScimGroupMemberPO> selectMembersByGroupIds(@Param("groupIds") List<Long> groupIds);
