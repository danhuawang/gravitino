...
import com.datastrato.gravitino.scim.web.rest.ScimAuthorizationFilter;
...
public class ScimTokenRESTFeature implements Feature {
  ...
  static void validateConfiguration(Config config) {
    ...
    if (!isScimPrincipalMapperConfigured(config)) {
      LOG.error(
          "gravitino.authenticator.oauth.principalMapper must be {} for SCIM.",
          ScimOAuthPrincipalMapper.PRINCIPAL_MAPPER_CLASS_NAME);
      System.exit(1);
    }
    if (!usesEmptyGroupsFields(config)) {
      LOG.error("gravitino.authenticator.oauth.groupsFields must be empty for SCIM.");
      System.exit(1);
    }
