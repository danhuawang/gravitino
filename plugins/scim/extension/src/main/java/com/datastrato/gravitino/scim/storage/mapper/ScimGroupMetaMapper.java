...
public interface ScimGroupMetaMapper {
  ...
  /**
   * Selects an active group by case-insensitive {@code group_name}.
   *
   * @param groupName group name to match ignoring case
   * @return matching row, or {@code null} when absent
   */
  @SelectProvider(
      type = ScimGroupMetaSQLProviderFactory.class,
      method = "selectByGroupNameIgnoreCase")
  ScimGroupMetaPO selectByGroupNameIgnoreCase(@Param("groupName") String groupName);
