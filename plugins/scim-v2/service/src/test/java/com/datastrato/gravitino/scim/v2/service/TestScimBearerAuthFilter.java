...
class TestScimBearerAuthFilter {
  ...
  void testBypassHealth() throws Exception {
    ...
    verify(tokenManager, never()).authenticateBearerToken(anyString());
  ...
  void testExpired419() throws Exception {
    ...
    doThrow(new TokenExpiredException("expired"))
        .when(tokenManager)
        .authenticateBearerToken("gravitino_scim_expired");
    filter.doFilter(request, response, chain);
  ...
  void testNotFound404() throws Exception {
    ...
    doThrow(new NotFoundException("missing"))
        .when(tokenManager)
        .authenticateBearerToken("gravitino_scim_test");
    filter.doFilter(request, response, chain);
  ...
  void testBadToken401() throws Exception {
    ...
    doThrow(new UnauthorizedException("bad")).when(tokenManager).authenticateBearerToken("bad");
    filter.doFilter(request, response, chain);
    verify(response).setStatus(HttpServletResponse.SC_UNAUTHORIZED);
  }
  ...
  private void stubAuth(String rawToken, ScimToken token) throws Exception {
    ...
    if (token != null) {
      when(tokenManager.authenticateBearerToken(rawToken)).thenReturn(token);
    }
  }
