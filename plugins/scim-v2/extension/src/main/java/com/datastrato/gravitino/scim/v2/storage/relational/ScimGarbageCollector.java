...
import com.datastrato.gravitino.scim.v2.storage.service.ScimErrorHistoryMetaService;
import com.datastrato.gravitino.scim.v2.storage.service.ScimGroupMetaService;
import com.datastrato.gravitino.scim.v2.storage.service.ScimTokenMetaService;
...
/** Garbage collector for SCIM v2 metadata expiry and purge. */
public final class ScimGarbageCollector implements Closeable {
  ...
  private final ScheduledExecutorService garbageCollectorPool =
      new ScheduledThreadPoolExecutor(
          2,
          r -> {
            Thread t = new Thread(r, "ScimV2-Garbage-Collector");
            t.setDaemon(true);
            return t;
          },
  ...
  private final ScheduledExecutorService errorHistoryCleaner =
      new ScheduledThreadPoolExecutor(
          1,
          r -> {
            Thread t = new Thread(r, "ScimV2-Error-History-Cleaner");
            t.setDaemon(true);
            return t;
          },
          new ThreadPoolExecutor.AbortPolicy());

  /** Creates a garbage collector for SCIM v2 metadata. */
  ...
  void collectAndClean() {
    ...
    LOG.debug("Thread {} start to collect SCIM v2 legacy garbage...", threadId);
    try {
      ...
      LOG.error("Thread {} failed to collect and clean SCIM v2 legacy garbage.", threadId, e);
    } finally {
      LOG.debug("Thread {} finish to collect SCIM v2 legacy garbage.", threadId);
    }
  }

  void cleanExpiredErrorHistory() {
    long threadId = Thread.currentThread().getId();
    LOG.debug("Thread {} start to clean expired SCIM v2 error history...", threadId);
    try {
      ...
      LOG.error("Thread {} failed to clean expired SCIM v2 error history.", threadId, e);
    } finally {
      LOG.debug("Thread {} finish cleaning expired SCIM v2 error history.", threadId);
    }
  }

  void softDeleteExpiredTokens() {
    long threadId = Thread.currentThread().getId();
    LOG.debug("Thread {} start to soft-delete expired SCIM v2 tokens...", threadId);
    try {
      int expiredDeleted = TOKEN_META_SERVICE.softDeleteExpiredScimTokens();
      if (expiredDeleted > 0) {
        LOG.info("Soft-deleted {} expired SCIM v2 token rows", expiredDeleted);
      }
    } catch (Exception e) {
      LOG.error("Thread {} failed to soft-delete expired SCIM v2 tokens.", threadId, e);
    } finally {
      LOG.debug("Thread {} finish soft-deleting expired SCIM v2 tokens.", threadId);
    }
  }
  ...
  void softDeleteOrphanMemberships() {
    long threadId = Thread.currentThread().getId();
    LOG.debug("Thread {} start to soft-delete orphan SCIM v2 memberships...", threadId);
    try {
      int deleted = USER_GROUP_REL_META_SERVICE.softDeleteOrphanMemberships();
      if (deleted > 0) {
        LOG.info("Soft-deleted {} orphan SCIM v2 membership rows", deleted);
      }
    } catch (Exception e) {
      LOG.error("Thread {} failed to soft-delete orphan SCIM v2 memberships.", threadId, e);
    } finally {
      LOG.debug("Thread {} finish soft-deleting orphan SCIM v2 memberships.", threadId);
    }
  }
  ...
  private static void purgeLegacyData(LegacyDataDeleter deleter) {
    ...
    try {
      ...
      LOG.error("Failed to physically delete SCIM v2 legacy data", e);
    }
  }
