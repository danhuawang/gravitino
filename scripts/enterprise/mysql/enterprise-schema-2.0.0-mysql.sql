...
CREATE TABLE IF NOT EXISTS `v2_scim_token_meta` (
    ...
    PRIMARY KEY (`token_id`),
    UNIQUE KEY `uk_v2stm_tn_del` (`token_name`, `deleted_at`),
    UNIQUE KEY `uk_v2stm_hash_del` (`token_hash`, `deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'scim v2 token metadata';
...
CREATE TABLE IF NOT EXISTS `v2_scim_user_meta` (
    ...
    PRIMARY KEY (`user_id`),
    UNIQUE KEY `uk_v2sun_del` (`user_name`, `deleted_at`),
    UNIQUE KEY `uk_v2sue_del` (`external_id`, `deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'scim v2 user metadata';
...
CREATE TABLE IF NOT EXISTS `v2_scim_group_meta` (
    ...
    UNIQUE KEY `uk_v2sgn_del` (`group_name`, `deleted_at`),
    UNIQUE KEY `uk_v2sge_del` (`external_id`, `deleted_at`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'scim v2 group metadata';
...
CREATE TABLE IF NOT EXISTS `v2_scim_user_group_rel` (
    ...
    `deleted_at` BIGINT(20) UNSIGNED NOT NULL DEFAULT 0 COMMENT 'relation deleted at',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_v2sugr_ui_gi_del` (`user_id`, `group_id`, `deleted_at`),
    KEY `idx_v2sugr_uid` (`user_id`),
    KEY `idx_v2sugr_gid` (`group_id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'scim v2 user group relation';
...
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'scim v2 protocol error history';
