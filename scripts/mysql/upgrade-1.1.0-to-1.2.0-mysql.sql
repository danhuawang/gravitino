...
CREATE TABLE IF NOT EXISTS `function_version_info` (
    ...
    `function_comment` MEDIUMTEXT DEFAULT NULL COMMENT 'function version comment',
...
CREATE TABLE IF NOT EXISTS `partition_statistic_meta` (
    `table_id` BIGINT(20) NOT NULL COMMENT 'table id from table_meta',
    ...
    `audit_info` MEDIUMTEXT NOT NULL COMMENT 'audit information as JSON',
    `created_at` BIGINT(20) NOT NULL COMMENT 'creation timestamp in milliseconds',
    `updated_at` BIGINT(20) NOT NULL COMMENT 'last update timestamp in milliseconds',
    PRIMARY KEY (`table_id`, `partition_name`, `statistic_name`),
    KEY `idx_table_partition` (`table_id`, `partition_name`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_bin COMMENT 'partition statistic metadata';
