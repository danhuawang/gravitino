 ...
 * / S3 secrets). Other properties, including reserved-but-visible ones (for example {@code
 * numFiles}, {@code creator}), are returned as-is. Reserved only controls whether users may write
 * the property; it does not remove keys from the response.
 *
...
public final class HiddenPropertyMaskUtils {
   ...
   * PropertiesMetadata#isHiddenProperty(String)} is true (credential and other sensitive keys).
   * Reserved keys are not removed.
   */
  public static Map<String, String> maskHiddenProperties(
    ...
    for (Map.Entry<String, String> entry : properties.entrySet()) {
      ...
      result.put(key, metadata.isHiddenProperty(key) ? MASKED_VALUE : value);
    }
    return result;
  }
}
